...
def main_pytorch_model():
    ...
    global X, X_kph, y_all, y_steering, y_throttle
    start_time = time.time()
    model = DAVE2v3(input_shape=(135,240))
    ...
    iteration = f'{model._get_name()}-lr1e4-{NB_EPOCH}epoch-batch{BATCH_SIZE}-lossMSE-{round(dataset.get_total_samples()/1000)}Ksamples-135x240-NOROBUSTIFICATION'
    ...
    for epoch in range(NB_EPOCH):  # loop over the dataset multiple times
        ...
        model_name = f"model-{iteration}-epoch{epoch}.pt"
